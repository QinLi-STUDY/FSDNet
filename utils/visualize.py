...
def export_sdas_images(config, fileinfos, gen_images, mrrn_images,epoch):

    export_root=os.path.join(config.vis_path,str(epoch))
    os.makedirs(export_root,exist_ok=True)

    for info,gen_image,mrrn_image in tqdm(zip(fileinfos,gen_images,mrrn_images)):
        ...
        mrrn_image= np.clip((mrrn_image.transpose(1,2,0) +1)* 127.5, a_min=0, a_max=255)

        merge_image = np.concatenate([ori_image,mrrn_image], axis=0).astype(np.uint8)
