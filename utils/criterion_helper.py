...
class FeatureMSELoss(nn.Module):
    ...
    def forward(self, input):
        feats_mrrn = input["feats_mrrn"]
        if "gt_block_feats" in input:
            gt_block_feats = input["gt_block_feats"]
            losses=[]
            for key in feats_mrrn:
                losses.append(self.criterion_mse(feats_mrrn[key], gt_block_feats[key]))
...
class ImageMSELoss(nn.Module):
    """Train a decoder for visualization of mrrnstructed features"""
