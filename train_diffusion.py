...
def SDAS_sample(imgs,class_labels,model,sampler):
    ...
    x_mrrn=torch.cat([imgs,xt_det,x0_det],dim=3)

    return x_mrrn,x_gen,x0_det
...
def main():
    ...
    for epoch in range(last_epoch, config.trainer.max_epoch):
        ...
        if (epoch + 1) % config.trainer.val_freq_epoch == 0:
            fileinfos, gen_images, mrrn_images, avg_loss = validate(config, val_loader, ema_model, test_sampler, epoch + 1)
            export_sdas_images(config, fileinfos, gen_images, mrrn_images, epoch+1)
            if rank == 0:
                save_checkpoint(
                                {
                                        "epoch": epoch + 1,
...
def validate(config, val_loader, model, test_sample, epoch):
    ...
    with torch.no_grad():
        for i, input in enumerate(val_loader):
            ...
            x_mrrn, x_gen, x0_det = SDAS_sample(imgs, class_labels, model, test_sample)
            ...
            x_mrrn_images.append(x_mrrn)

            l1 = []
            for name, criterion_loss in criterion.items():
                weight = criterion_loss.weight
                l1.append(weight * criterion_loss({"ori": imgs, "mrrn": x0_det}))
    ...
    mrrn_images = torch.cat(x_mrrn_images, dim=0).cpu().detach().numpy()

    return fileinfos, gen_images, mrrn_images, avg_loss
