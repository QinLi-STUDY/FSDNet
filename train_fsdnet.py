...
# CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.launch --nproc_per_node=1 train_fsdnet.py --dataset MVTec-AD --class_name bottle

warnings.filterwarnings('ignore')
parser = argparse.ArgumentParser(description="train FSDNet")
parser.add_argument("--config", default="experiments/{}/fsdnet.yaml")
parser.add_argument("--dataset", default="MVTec-AD",choices=['MVTec-AD','VisA','MPDD','BTAD'])
parser.add_argument("--local_rank", default=-1, type=int)

parser.add_argument("--class_name", default="wood",choices=[
        # mvtec-ad
        "bottle",
        "cable",
        "capsule",
...
def main():
    ...
    rank, world_size = setup_distributed()# rank 0, world_size 1
    # rank = 0
    set_seed(config.random_seed)
    config = update_config(config,args)

    config.exp_path = os.path.dirname(args.config)# 'experiments/MVTec-AD'
    config.checkpoints_path = os.path.join(config.exp_path, config.saver.checkpoints_dir)# 'experiments/MVTec-AD/fsdnet_checkpoints/'
    config.log_path = os.path.join(config.exp_path, config.saver.log_dir)# 'experiments/MVTec-AD/fsdnet_log/'
    ...
    if rank==0:
        ...
        logger = create_logger(
            "fsdnet_logger_{}".format(args.class_name), config.log_path + "/fsdnet_{}_{}.log".format(args.class_name,current_time)
        )
    ...
    model.afpm.init_idxs(model, train_loader, distributed=True)
    ...
    frozen_layers = model.module.frozen_layers# backbone 
    ...
    optimizer = get_optimizer(parameters, config.trainer.optimizer) # {'lr': 0.0001, 'betas': [0.9, 0.999], 'eps': 1e-08, 'weight_decay': 0, 'amsgrad': False, 'maximize': False}

    key_metric = config.evaluator["key_metric"] # mean

    best_metric = 0
    last_epoch = 0

    criterion = build_criterion(config.criterion) # SegmentCrossEntropyLoss FeatureMSELoss

    for epoch in range(last_epoch, config.trainer.max_epoch):

        last_iter = epoch * len(train_loader) # 0
        ...
        if (epoch + 1) % config.trainer.val_freq_epoch == 0:
            ...
            if rank==0:
                ...
                log_metrics(ret_metrics, config.evaluator.metrics, "fsdnet_logger_{}".format(args.class_name))
                if is_best:
                    save_checkpoint(
                        {
                            "epoch": epoch + 1,
...
def train_one_epoch(
    ...
    if rank == 0:
        logger = logging.getLogger("fsdnet_logger_{}".format(class_name))
