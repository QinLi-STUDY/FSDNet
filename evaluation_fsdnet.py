...
from train_fsdnet import update_config
from utils.categories import Categories


warnings.filterwarnings('ignore')
parser = argparse.ArgumentParser(description="evaluation fsdnet")
...
def main():
    ...
    config.evaluator.metrics['auc'].append({'name':'appx'})
    config.evaluator.metrics['auc'].append({'name':'apsp'})
    config.evaluator.metrics['auc'].append({'name':'f1px'})
    config.evaluator.metrics['auc'].append({'name':'f1sp'})
...
def validate(config,val_loader, model,class_name):
    ...
    # print(preds.shape)
    preds = np.squeeze(np.concatenate(preds, axis=0),axis=1)  # N x H x W
    masks = np.squeeze(np.concatenate(masks, axis=0),axis=1)  # N x H x W
